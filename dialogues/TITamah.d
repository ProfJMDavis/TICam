...
+~PartyHasItem("TIC1SCRL")~ + ~There's this one. It's printed with a border of roses..~ DO ~DestroyItem("TIC1SCRL")~ + a3b
+~PartyHasItem("TIC2SCRL")~ + ~There's this one written in gold ink.~ DO ~DestroyItem("TIC2SCRL")~ + a3c
+~PartyHasItem("TIC3SCRL")~ + ~There's this one. It's got some little drawings of cherubs on it.~ DO ~DestroyItem("TIC3SCRL")~ + a3d
+~PartyHasItem("TIC4SCRL")~ + ~There's this one printed on blue paper.~ DO ~DestroyItem("TIC4SCRL")~ + a3e
+~PartyHasItem("TIC5SCRL")~ + ~There's this one illuminated with gold leaf.~ DO ~DestroyItem("TIC5SCRL")~ + a3f
+~PartyHasItem("TIC6SCRL")~ + ~There's this one... He must have put perfume on the paper... It smells like sandalwood and jasmine.~ DO ~DestroyItem("TIC6SCRL")~ + a3g
+~PartyHasItem("TIC7SCRL")~ + ~This is the prettiest one; he uses gold ink AND paper printed with a border of roses on linen paper.~ DO ~DestroyItem("TIC7SCRL")~ + a3h
+~PartyHasItem("TIC8SCRL")~ + ~This one feels a bit thicker than the rest.~ DO ~DestroyItem("TIC8SCRL")~ + a3i
+~PartyHasItem("TIC9SCRL")~ + ~This one has pictures of books on it.~ DO ~DestroyItem("TIC9SCRL")~ + a3j
+~PartyHasItem("TIC10SCR")~ + ~This one is written on vellum.~ DO ~DestroyItem("TIC10SCR")~ + a3k
+~PartyHasItem("TIC11SCR")~ + ~Oh my... This one's printed on linen, but it's as smooth as silk and it smells of perfume, too.~ DO ~DestroyItem("TIC11SCR")~ + a3l
+~PartyHasItem("TIC12SCR")~ + ~This one has entwined gold rings drawn on it.~ DO ~DestroyItem("TIC12SCR")DestroyItem("TICAMRIN")~ + a3m
+~PartyHasItem("TIC13SCR")~ + ~Huh! This one looks like a party invitation.~ DO ~DestroyItem("TIC13SCR")~ + a3n
